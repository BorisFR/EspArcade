let baseUrl = 'https://borisfr.github.io/RayLibArcade/';
...
function populatePopup(game) {
    ...
    document.getElementById("gameTitle").innerHTML = "<img src=\""+baseUrl+"./sdcard/_icon/" + game + ".png\" alt=\"" + info["name"] + "\" /> " + info["name"];
    document.getElementById("gameScreen").src = baseUrl +"./sdcard/ss/" + game + ".png";
...
function openPopup(game) {
    if (!gamesInfo.has(game)) {
        let name = baseUrl +'./wiki/' + game + ".json";
        fetch(name)
            .then(response => {
...
function createLink(element) {
    var x = "<div onclick=\"openPopup('" + element["id"] + "')\"><img src=\""+baseUrl+"./sdcard/_icon/" + element["id"] + ".png\" alt=\"" + element["name"] + "\" /> <span class=\"gameTitle\">" + element["name"] + "</span></div>";
    return x;
};
function dolist() {
    console.log("loading");
    fetch(baseUrl+'./wiki/gamelist.json')
        .then(response => {
            if (!response.ok) { throw new Error('HTTP Error Status: ' + response.status); }
            return response.json();
        })
